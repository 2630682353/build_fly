...
--	the parameter is : tag name , type of textfiled , config file option , display before the filed
	mcsv = s:taboption("advanced", ListValue, "mcs", translate("Bitrates Setting (MCS)"))
	for i=0,15 do
		mcsv:value(i, "MCS-"..i);
	end	
end
...
e = s:taboption("general", Value, "ssid", translate("<abbr title=\"Extended Service Set Identifier\">ESSID</abbr>"),translate("no more than 10 chinese word while 32 latin letters at most"))
e.datatype = "ssid"
...
function m.on_before_save()
	...
	if wnet:get("encryption") ~= "none" and  wnet:get("key") == nil then
		m.uci:unload("wireless")
		m.message = translate("Some input is null,the configuration don't apply !")
	end
end
