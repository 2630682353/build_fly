...
if not net then
	luci.http.redirect(luci.dispatcher.build_url("admin/quick/wan_quick"))
	return
end
...
s = m:section(NamedSection, arg[1], "interface", translate(""))
s.addremove = false
...
local form, ferr = loadfile(
	ut.libpath() .. "/model/cbi/admin_quick/proto_%s.lua" % net:proto()
)
...
function m.on_before_save()
	if net:proto() == "static" then
		if  m.uci:get ("network","wan","ipaddr") == m.uci:get ("network","lan","ipaddr") then
			m.message = translate("IP address is wrong,the configuration don't apply !")
			m.uci:unload("network")
		end
	end
end
