...
function m.on_commit(map)
	...
	if tonumber(m.uci:get("wireless","radio0", "channel")) > 11 then
		luci.sys.call("uci set wireless.radio0.hwmode=11a")
		luci.sys.call("uci commit")
	else 
		luci.sys.call("uci set wireless.radio0.hwmode=11g")
		luci.sys.call("uci commit")
	end
end
...
s = m:section(NamedSection, wdev:name(), "wifi-device", translate(""))
s.addremove = false
...
--[[
	mode = s:taboption("advanced",DummyValue,"hwmode",translate("Mode"))
	mode.value = ("11ng")
...
--		if hw_modes.a and hw_modes.n then mode:value("11na", "802.11a+n") end
	...
	if cl and #cl > 0 then
		...
		cc:value("CN",translate("CN - China"))
...
s = m:section(NamedSection, wnet.sid, "wifi-iface", translate(""))
ifsection = s
s.addremove = false
s.anonymous = true
...
ESSID =  s:taboption("general", Value, "ssid", translate("<abbr title=\"Extended Service Set Identifier\">ESSID</abbr>"),translate("no more than 10 chinese word while 32 latin letters at most"))
ESSID.datatype = "ssid"
...
function m.on_before_save()
	if wnet:get("ssid") == nil  then
		m.message = translate("Some input is null,the configuration don't apply !")
		m.uci:unload("wireless")
	end
end
