...
function index()
	...
	if page.inreq then
		...
		if has_wifi then
			...
			page = entry({"admin", "network", "wireless"}, arcombine(template("admin_network/wifi_overview"), cbi("admin_network/wifi")), _("Wifi"), 3)
			page.leaf = true
			page.subindex = true
		...
		page = entry({"admin", "network", "lan"}, arcombine(cbi("admin_network/network"), cbi("admin_network/ifaces")),"LAN",1)
		page.leaf = true  

		page = entry({"admin", "network", "wan"}, arcombine(cbi("admin_network/network1"), cbi("admin_network/ifaces")),"WAN",2)
		page.leaf = true  
...
function iface_delete(iface)
	...
	if net then
		luci.sys.call("env -i /sbin/ifdown %q >/dev/null 2>/dev/null" % iface)
		if iface ~= "loopback" and iface ~= "wan" and iface ~= "wan1" and iface ~= "wan2" and iface ~= "wan3" and iface ~= "wan4" and iface ~= "wan6" then
